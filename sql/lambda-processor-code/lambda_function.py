...
def lambda_handler(event, context):
    ...
    for record in event['records']:
        ...
        document = {
            'session_id': msn['SESSION_ID'],
            'user_id': msn['USER_ID'],
            'device_Id': msn['DEVICE_ID'],
            'timeagg': msn['TIMEAGG'],
            'events': msn['EVENTS'],
            'beginnavigation': msn['BEGINNAVIGATION'],
            'endnavigation': msn['ENDNAVIGATION'],
            'beginsession' : msn['BEGINSESSION'],
            'endsession' : msn['ENDSESSION'],
            'duration_sec': msn['DURATION_SEC']
        }
        documentjson = str(document) + '\n'
        fh.put_record(DeliveryStreamName=DELIVERY_STREAM_NAME,Record={ 'Data': documentjson})
        cw_write = {
